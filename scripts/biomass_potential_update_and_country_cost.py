...
def create_biomass_costs(output_folder,config = 'config/config.yaml', scenario = 'ENS_Med', year =2050, database_file ='data/ENSPRESO_BIOMASS.xlsx'):
    ...
    weighted_avg_csv_filename = f'{output_folder}/weighted_average_costs_{year}_{scenario}.csv'
    ...
    for group, types in biomass_classes.items():
        ...
        if not group_df.empty:
            weighted_avg_cost = np.average(group_df['NUTS0 Energy Commodity Cost '], weights=group_df['Value'])
            total_potential = group_df['Value'].sum()
            grouped_costs.append({'Group': group, 'Weighted Average Cost': weighted_avg_cost, 'Total Potential': total_potential})

    grouped_costs_df = pd.DataFrame(grouped_costs)

    # Create the CSV file for grouped weighted average costs
    grouped_weighted_avg_csv_filename = f'{output_folder}/grouped_weighted_average_costs_{year}_{scenario}.csv'
    grouped_costs_df.to_csv(grouped_weighted_avg_csv_filename, index=False)

if __name__ == "__main__":
    create_biomass_costs('resources/')
