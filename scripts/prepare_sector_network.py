...
def add_biomass(n, costs):
    ...
    for carrier in biomass_types:
        n.madd(
            "Store",
            spatial_dict[carrier],
            bus=spatial_dict[carrier],
            carrier=carrier,
            e_nom=potentials_dict[carrier],
            marginal_cost=costs.at["solid biomass", "fuel"],
            e_initial=potentials_dict[carrier],
        )
    ...
    if options["biomass_transport"]:
        ...
        for carrier in biomass_types:
            n.madd(
                ...
                bus0=biomass_transport.bus0 + f" {carrier}",
                bus1=biomass_transport.bus1 + f" {carrier}",
                p_nom_extendable=False,
                p_nom=5e4,
                length=biomass_transport.length.values,
                marginal_cost=biomass_transport.costs * biomass_transport.length.values,
                carrier=f"{carrier} transport",
            )
        ...
        for carrier in biomass_types:
           ...
           n.add(
            ...
            sense="<=",
            constant=potentials_dict[carrier].sum(),
    ...
    if not urban_central.empty and options["chp"]:
        ...
        for carrier in biomass_types:
            n.madd(
                ...
                bus3="co2 atmosphere",
                carrier=f"urban central {carrier} CHP",
                p_nom_extendable=True,
                capital_cost=costs.at[key, "fixed"] * costs.at[key, "efficiency"],
                marginal_cost=costs.at[key, "VOM"],
                efficiency=costs.at[key, "efficiency"],
                efficiency2=costs.at[key, "efficiency-heat"],
                efficiency3= biomass_ef.get(carrier),
                lifetime=costs.at[key, "lifetime"],
            )
            n.madd(
                "Link",
                ...
                carrier=f"urban central {carrier} CHP CC",
                p_nom_extendable=True,
                capital_cost=costs.at[key, "fixed"] * costs.at[key, "efficiency"]
                + costs.at["biomass CHP capture", "fixed"]
                ...
                * costs.at["biomass CHP capture", "capture_rate"] + biomass_ef.get(carrier),
    ...
    if options["biomass_to_liquid"]:
        for carrier in biomass_types:
            n.madd(
                "Link",
                spatial_dict[carrier],
                suffix=f" {carrier} to liquid",
                bus0=spatial_dict[carrier],
                bus1=spatial.oil.nodes,
                bus2="co2 atmosphere",
                carrier=f"{carrier} to liquid",
                lifetime=costs.at["BtL", "lifetime"],
                efficiency=costs.at["BtL", "efficiency"],
                efficiency2=-costs.at["solid biomass", "CO2 intensity"]
                + costs.at["BtL", "CO2 stored"] + biomass_ef.get(carrier),
                p_nom_extendable=True,
            ...
            n.madd(
                "Link",
                spatial_dict[carrier],
                suffix=f" {carrier} to liquid CC",
                bus0=spatial_dict[carrier],
                bus1=spatial.oil.nodes,
                bus2=spatial.co2.nodes,
                bus3="co2 atmosphere",
                carrier=f"{carrier} to liquid CC",
                lifetime=costs.at["BtL", "lifetime"],
                efficiency=costs.at["BtL", "efficiency"],
                efficiency2=costs.at["BtL", "CO2 stored"]
                * costs.at["BtL", "capture rate"],
                efficiency3=-costs.at["solid biomass", "CO2 intensity"]
                + costs.at["BtL", "CO2 stored"]
                * (1 - costs.at["BtL", "capture rate"])+ biomass_ef.get(carrier),
                p_nom_extendable=True,
                capital_cost=costs.at["BtL", "fixed"]
                + costs.at["biomass CHP capture", "fixed"]
                * costs.at["BtL", "CO2 stored"],
    ...
    if options["biosng"]:
        for carrier in biomass_types:
            n.madd(
                "Link",
                spatial_dict[carrier],
                suffix=f" {carrier} to gas",
                bus0=spatial_dict[carrier],
                bus1=spatial.gas.nodes,
                bus2="co2 atmosphere",
                carrier=f"BioSNG from {carrier}",
                lifetime=costs.at["BioSNG", "lifetime"],
                efficiency=costs.at["BioSNG", "efficiency"],
                efficiency2=-costs.at["solid biomass", "CO2 intensity"]
                + costs.at["BioSNG", "CO2 stored"] + biomass_ef.get(carrier),
                p_nom_extendable=True,
            ...
            n.madd(
                "Link",
                spatial_dict[carrier],
                suffix=f" {carrier} to gas CC",
                bus0=spatial_dict[carrier],
                bus1=spatial.gas.nodes,
                bus2=spatial.co2.nodes,
                bus3="co2 atmosphere",
                carrier=f"BioSNG from {carrier} CC",
                lifetime=costs.at["BioSNG", "lifetime"],
                efficiency=costs.at["BioSNG", "efficiency"],
                ...
                * (1 - costs.at["BioSNG", "capture rate"])+ biomass_ef.get(carrier),
                p_nom_extendable=True,
                capital_cost=costs.at["BioSNG", "fixed"]
                + costs.at["biomass CHP capture", "fixed"]
                * costs.at["BioSNG", "CO2 stored"],
...
def add_industry(n, costs):
    ...
    for carrier in biomass_types:
        n.madd(
            ...
            bus2="co2 atmosphere",
            bus3=spatial.co2.nodes,
            carrier=f"{carrier} for industry CC",
            p_nom_extendable=True,
            capital_cost=costs.at["cement capture", "fixed"]
            * costs.at["solid biomass", "CO2 intensity"],
            efficiency=0.9,  # TODO: make config option
            efficiency2=-costs.at["solid biomass", "CO2 intensity"]
            * costs.at["cement capture", "capture_rate"] + biomass_ef.get(carrier),
            efficiency3=costs.at["solid biomass", "CO2 intensity"]
